...
public interface LastYearScoreRepository extends JpaRepository<LastYearScore, String> {
    ...
    @Query(value = "select count(id) from kb_last_year_score where class_category=?1 and score=?2 and tenant_id =?3", nativeQuery = true)
    Integer getCountScoreByClassCategory(String classCategory, int score, String tenantId);

    @Query(value = "select DISTINCT score from kb_last_year_score where tenant_id =?1 ORDER BY score", nativeQuery = true)
    List<Integer> getAllScore(String tenantId);
